...
USER_AGENT = "healthtech-dashboard/1.0 (+news collector)"
...
def collect(start: date, end: date) -> list[Article]:
    ...
    Degrades gracefully: a feed that's unreachable or unparsable is logged
    and skipped rather than raising, so one dead feed never fails the run.
    """
