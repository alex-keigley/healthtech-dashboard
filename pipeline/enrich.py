"""Populate `company_descriptions` for the most newsworthy recent companies.
...
Source priority:
  ...
  2. A meta description scraped from the company's website (resolved via
     DuckDuckGo). Adds latency, so we only fall back here when news is
     missing AND the company looks important enough to be worth showing.
...
are cheap. To force a refresh, delete the row from `company_descriptions`.
"""
...
def _has_description(conn, company_id: int) -> bool:
    row = conn.execute(
        "SELECT 1 FROM company_descriptions WHERE company_id = ?", (company_id,)
    ).fetchone()
    ...
    *,
    ...
    for r in rows:
        ...
        if info:
            ...
            from_web += 1
        # Politeness pause even on miss so we don't hammer DDG.
        time.sleep(delay)
